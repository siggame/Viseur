...
export class Pane extends BasePane<GameState, PlayerState> {
    ...
    /**
     * Creates the pane for the Chess game.
     *
    ...
    protected getGameStats(
        ...
        stats.push({
            get: (game) => {
                const move = Math.max(
                    Math.ceil((game.history.length - 1) / 2),
                    0,
                );
                const sideToMove =
    ...
    /**
     * Override to get the current player's id because this is not a
     * TurnBasedGame based game class.
     *
     * @param state - The current game state.
     * @returns The id of the current player.
     */
    protected getCurrentPlayerID(state: Immutable<GameState>): string {
